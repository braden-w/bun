    ...
    pub fn toStringBoth(self: *const Self, allocator: std.mem.Allocator) !StringPair {
        ...
        const fetch_spec_with_prefix = switch (self.default_representation) {
            ...
            .git => blk: {
                const git_formatter = HostProvider.configs.get(self.host_provider).format_git;
                if (git_formatter) |formatter| {
                    break :blk try formatter(self.host_provider, allocator, null, self.user, self.project, null);
                }
                @panic("No git formatter for this provider. This is a bug in Bun.");
            },
            .http => @panic("No http formatter exists. This is a bug in Bun."),
        ...
        const has_git_plus = bun.strings.hasPrefixComptime(fetch_spec_with_prefix, "git+");
        if (has_git_plus) {
            const stripped = fetch_spec_with_prefix[4..];
            const fetch_spec = try allocator.dupe(u8, stripped);
            allocator.free(fetch_spec_with_prefix);
            return .{
                .save_spec = save_spec,
        ...
        return .{
            .save_spec = save_spec,
            .fetch_spec = fetch_spec_with_prefix,
        };
    }
...
fn normalizeProtocol(npa_str: []u8) UrlProtocolPair {
    ...
    if (maybe_dup_slash_idx) |dup_slash_idx| {
        if (dup_slash_idx == first_colon_idx + 1) {
            return .{
                .url = bun.strings.substring(npa_str, dup_slash_idx + 2, null),
    ...
    if (first_colon_idx != -1) {
        return .{
            .url = bun.strings.substring(npa_str, @intCast(first_colon_idx + 1), null),
